...
const CameraStatusCard = ({ camera }: CameraStatusCardProps) => (
  <article className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
    ...
    <p className="mt-2 text-xs text-gray-500">{camera.location}</p>
    <p className="mt-2 text-xs text-gray-500">Ultima actividad: {new Date(camera.lastUpdate).toLocaleString()}</p>
  </article>
);
