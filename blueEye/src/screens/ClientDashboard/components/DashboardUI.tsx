  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 9, fontFamily: T.mono, letterSpacing: "0.14em", textTransform: "uppercase" as const, color: T.t3 }}>
          {label}
        </span>
        {dotColor && <div style={{ width: 6, height: 6, borderRadius: "50%", background: dotColor }} />}
      </div>
      <div style={{ fontSize: 30, fontWeight: 300, letterSpacing: "-0.02em", lineHeight: 1, color: valueColor, fontFamily: T.font, fontStyle: "normal" }}>
        {value}
      </div>
      <div style={{ fontSize: 10, color: T.t3, fontFamily: T.font }}>{detail}</div>
      {barWidth && barFill && (
        <div style={{ height: 2, background: "#1C2F47", borderRadius: 1, overflow: "hidden", marginTop: 2 }}>
          <div style={{ height: "100%", width: barWidth, background: barFill, borderRadius: 1 }} />
        </div>
      )}
    </div>
  );
...
const ALERT_S = {
  critical: { bar: "#B05040", title: "#C07060", tagBg: "#2A1510", tagColor: "#C07060", tagBd: "#3A1A14" },
  warning:  { bar: "#C09040", title: "#C09840", tagBg: "#261D0A", tagColor: "#C09840", tagBd: "#3A2A08" },
  info:     { bar: "#4878A0", title: "#7099C0", tagBg: "#0E2038", tagColor: "#7099C0", tagBd: "#1A3858" },
};
...
const AlertRow: React.FC<{ alert: DashboardAlert; isLast?: boolean }> = ({ alert, isLast }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 500, color: s.title, marginBottom: 3, fontFamily: T.font, fontStyle: "normal" }}>
          {alert.title}
        </div>
        <div style={{ fontSize: 11, color: T.t2, lineHeight: 1.5, fontFamily: T.font }}>
          {alert.description}
        </div>
      </div>
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        <span style={{ display: "block", fontSize: 9, fontFamily: T.mono, color: T.t3, letterSpacing: "0.06em", marginBottom: 5 }}>
          {alert.time}
        </span>
        <span style={{ fontSize: 9, fontFamily: T.mono, padding: "2px 7px", borderRadius: 4, background: s.tagBg, color: s.tagColor, border: `1px solid ${s.tagBd}`, whiteSpace: "nowrap" as const }}>
          {alert.source}
        </span>
      </div>
    </div>
...
const Panel: React.FC<{
  ...
  <div style={{
    ...
    <div style={{
      ...
      <div>
        <div style={{ fontSize: 14, fontWeight: 600, color: T.t1, letterSpacing: "-0.01em", fontFamily: T.font, fontStyle: "normal" }}>{title}</div>
        <div style={{ fontSize: 9, fontFamily: T.mono, letterSpacing: "0.12em", textTransform: "uppercase" as const, color: T.t3, marginTop: 2 }}>{sub}</div>
      </div>
      {right}
    </div>
    {children}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontSize: 10, fontFamily: T.mono, letterSpacing: "0.16em", textTransform: "uppercase" as const, color: T.mint, marginBottom: 5 }}>
            Portal del cliente · Infraestructura
          </div>
          <h1 style={{ fontSize: 22, fontWeight: 600, color: T.t1, letterSpacing: "-0.02em", margin: 0, fontFamily: T.font, fontStyle: "normal", lineHeight: 1.2 }}>
            Estado del Sistema
          </h1>
          <p style={{ fontSize: 11, color: T.t3, marginTop: 4, fontFamily: T.font }}>
            Monitoreo en tiempo real · {cameras.length} cámaras instaladas
          </p>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ fontSize: 20, fontWeight: 300, fontFamily: T.mono, color: T.t1, letterSpacing: "0.04em" }}>
            {clock}
          </div>
      ...
      {alerts.length > 0 && (
        <div style={{
          ...
          <span style={{ color: "#B05040", fontSize: 11, fontWeight: 600, flexShrink: 0 }}>▲</span>
          <span style={{ fontSize: 11, color: "#C07060", flex: 1, fontFamily: T.font }}>
            {alerts.length} incidentes activos —{" "}
            {alerts.map(a => a.source).join(" · ")}
          </span>
          <Tag bg="#1A0A08" color="#A05040" border="#4A2018">{alerts.length} ALERTAS</Tag>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
        <StatCard
          ...
          accentBorder="#3D9664" valueColor={T.mint} dotColor={T.mint}
          barFill="#3D9664" barWidth={`${(online / cameras.length) * 100}%`}
          animDelay={40}
        />
        <StatCard
          label="Cámaras Offline" value={offline}
          detail="requieren atención"
          accentBorder="#7A3020" valueColor="#B06050" dotColor="#B06050"
          animDelay={80}
        />
        <StatCard
          label="Mantenimiento"   value={maintenance}
          detail="en revisión técnica"
          accentBorder="#7A5C20" valueColor="#C09040" dotColor="#C09040"
          animDelay={120}
        />
        <StatCard
          label="Uptime · 30d"   value="98.2%"
          detail="últimos 30 días"
          accentBorder="#2A5280" valueColor="#6090C0"
          barFill="#2A5280" barWidth="98.2%"
          animDelay={160}
        />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        <Panel
          ...
          right={<Tag bg="#2A1510" color="#B06050" border="#3A1A14">{alerts.length} activas</Tag>}
        >
          {alerts.map((a, i) => (
        ...
        <Panel
          ...
          sub="Últimos 30 días"
          right={
            <span style={{ fontSize: 18, fontWeight: 300, fontFamily: T.mono, color: T.mint, letterSpacing: "0.04em" }}>
              98.2%
            </span>
          }
          style={{ gridColumn: "1 / -1" }}
          ...
          <div style={{ padding: "10px 16px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 40 }}>
              {UPTIME_DATA.map((v, i) => {
                ...
                const bg = isLast ? T.mint : v < 88 ? T.red : v < 96 ? T.amber : T.blue;
                return (
                  <div
                    key={i}
                    title={`Día ${i + 1}: ${v}%`}
                    ...
                    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.opacity = isLast ? "1" : "0.3"; }}
                  />
                );
              })}
            </div>
