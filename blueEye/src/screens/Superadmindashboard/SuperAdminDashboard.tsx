...
const estadoBadge = (estado: EstadoEmpresa): React.ReactElement => {
  ...
  return (
    <span style={{ background: s.bg, color: s.color, padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase" }}>
      {s.label}
    </span>
  );
};
...
const planBadge = (plan: TipoPlan): React.ReactElement => {
  ...
  return (
    <span style={{ color: map[plan] ?? "#94a3b8", fontWeight: 700, fontSize: 12 }}>{plan}</span>
  );
};
...
function StatCard({ icon, label, value, sub, accent = "#22d3ee", trend }: StatCardProps): React.ReactElement {
  return (
    <div
      ...
      onMouseEnter={(e: MouseEvent<HTMLDivElement>) => (e.currentTarget.style.borderColor = "rgba(34,211,238,0.28)")}
      onMouseLeave={(e: MouseEvent<HTMLDivElement>) => (e.currentTarget.style.borderColor = "rgba(34,211,238,0.08)")}
    >
      <div style={{ position: "absolute", top: -30, right: -30, width: 100, height: 100, borderRadius: "50%", background: accent, opacity: 0.04, filter: "blur(30px)", pointerEvents: "none" }} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <span style={{ fontSize: 22 }}>{icon}</span>
        {trend !== undefined && (
          <span style={{ fontSize: 11, fontWeight: 700, color: trend >= 0 ? "#22d3ee" : "#ef4444", background: trend >= 0 ? "rgba(34,211,238,0.1)" : "rgba(239,68,68,0.1)", padding: "2px 8px", borderRadius: 20 }}>
            {trend >= 0 ? "+" : ""}{trend}%
          </span>
        )}
      </div>
      <div style={{ fontSize: 30, fontWeight: 800, color: "#f1f5f9", fontFamily: "'Syne', sans-serif", lineHeight: 1 }}>{value}</div>
...
function UsageBar({ pct, color = "#22d3ee" }: UsageBarProps): React.ReactElement {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 99, height: 5, width: "100%", overflow: "hidden" }}>
...
const CustomTooltip = ({ active, payload, label }: CustomTooltipProps): React.ReactElement | null => {
  ...
  return (
    <div style={{ background: "#0f172a", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 10, padding: "10px 16px" }}>
      <p style={{ color: "#94a3b8", fontSize: 11, marginBottom: 4 }}>{String(label ?? "")}</p>
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color ?? "#fff", fontSize: 13, fontWeight: 700, margin: 0 }}>
          {p.name === "ingresos" ? "$" : ""}{Number(p.value ?? 0).toLocaleString()} {p.name}
        </p>
      ))}
    </div>
  );
...
export default function SuperAdminDashboard(): React.ReactElement {
  ...
  const quickActions: QuickAction[] = [
    ...
    { icon: "📧", label: "Email Masivo",   desc: "Comunicar a todas las empresas" },
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", background: "#060d1a" }}>

        {/* ── SIDEBAR ── */}
        <aside style={{ width: 220, background: "linear-gradient(180deg,#0a1628 0%,#070f1f 100%)", borderRight: "1px solid rgba(34,211,238,0.07)", display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
          <div style={{ padding: "28px 20px 24px", borderBottom: "1px solid rgba(34,211,238,0.07)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, color: "#f1f5f9", fontSize: 15, letterSpacing: "0.02em" }}>BlueEye</div>
                <div style={{ fontSize: 10, color: "#22d3ee", letterSpacing: "0.1em", fontWeight: 600, textTransform: "uppercase" }}>Super Admin</div>
              </div>
            </div>
          </div>
            ...
            {navItems.map((item) => (
              <div
                ...
                style={{
                  ...
                  background:  activeNav === item.id ? "rgba(34,211,238,0.1)" : "transparent",
                  borderLeft:  activeNav === item.id ? "2px solid #22d3ee"    : "2px solid transparent",
          ...
          <div style={{ padding: "16px 16px 20px", borderTop: "1px solid rgba(34,211,238,0.07)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ color: "#475569", fontSize: 10 }}>admin@blueeye.io</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "auto", animation: "fadeIn 0.4s ease" }}>
          ...
          <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 32px", borderBottom: "1px solid rgba(34,211,238,0.07)", background: "rgba(6,13,26,0.8)", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 10 }}>
            <div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 22, color: "#f1f5f9" }}>Dashboard Global</div>
              <div style={{ color: "#475569", fontSize: 12, marginTop: 2 }}>Visión general de toda la plataforma</div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(34,211,238,0.06)", border: "1px solid rgba(34,211,238,0.15)", borderRadius: 20, padding: "6px 14px" }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#22d3ee", animation: "pulse-dot 1.4s infinite" }} />
                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 11, color: "#22d3ee" }}>
                  {time.toLocaleTimeString("es-DO", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
                </span>
              </div>
              <div style={{ position: "relative", cursor: "pointer" }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>🔔</div>
                <div style={{ position: "absolute", top: 4, right: 4, width: 8, height: 8, borderRadius: "50%", background: "#ef4444", border: "2px solid #060d1a" }} />
              </div>
            </div>
          </header>
          ...
          <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: 28 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 300px", gap: 16 }}>
              ...
              <div style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", border: "1px solid rgba(34,211,238,0.08)", borderRadius: 16, padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: "#f1f5f9", fontSize: 14 }}>Crecimiento de Empresas</div>
                    <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>Últimos 7 meses</div>
                  </div>
                ...
                <ResponsiveContainer width="100%" height={160}>
                  <AreaChart data={growthData}>
                    ...
                    <XAxis dataKey="mes" stroke="#334155" tick={{ fill: "#475569", fontSize: 11 }} axisLine={false} tickLine={false} />
              ...
              <div style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", border: "1px solid rgba(34,211,238,0.08)", borderRadius: 16, padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: "#f1f5f9", fontSize: 14 }}>Ingresos por Suscripción</div>
                    <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>USD · Últimos 7 meses</div>
                  </div>
              ...
              <div style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", border: "1px solid rgba(34,211,238,0.08)", borderRadius: 16, padding: 24 }}>
                <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: "#f1f5f9", fontSize: 14, marginBottom: 4 }}>Distribución por Plan</div>
                <div style={{ color: "#475569", fontSize: 11, marginBottom: 16 }}>54 empresas totales</div>
                <div style={{ display: "flex", justifyContent: "center" }}>
                  <PieChart width={140} height={140}>
                    ...
                    <Tooltip contentStyle={{ background: "#0f172a", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 8, fontSize: 12 }} />
                  </PieChart>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
                  {planData.map((p) => (
                    <div key={p.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <span style={{ color: "#cbd5e1", fontSize: 12, fontWeight: 700 }}>{p.value}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16 }}>
              ...
              <div style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", border: "1px solid rgba(34,211,238,0.08)", borderRadius: 16, padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: "#f1f5f9", fontSize: 14 }}>Empresas Recientes</div>
                    <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>Últimas actividades</div>
                  </div>
                  <button style={{ fontSize: 11, color: "#22d3ee", background: "rgba(34,211,238,0.08)", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontWeight: 600 }}>
                    Ver todas →
                  </button>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 80px 80px 120px 60px", gap: 12, padding: "6px 12px", marginBottom: 4 }}>
                  {["Empresa", "Admin", "Plan", "Estado", "Uso", "Reg."].map((h) => (
                    <div key={h} style={{ fontSize: 10, color: "#334155", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>{h}</div>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {recentCompanies.map((c) => (
                    <div
                      key={c.id}
                      className="row-hover"
                      style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 80px 80px 120px 60px", gap: 12, padding: "10px 12px", borderRadius: 10, alignItems: "center", transition: "background 0.15s" }}
                    >
              ...
              <div style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)", border: "1px solid rgba(34,211,238,0.08)", borderRadius: 16, padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                  <div>
                    <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: "#f1f5f9", fontSize: 14 }}>Bitácora de Eventos</div>
                    <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>Últimas acciones críticas</div>
                  </div>
                  <button style={{ fontSize: 11, color: "#22d3ee", background: "rgba(34,211,238,0.08)", border: "1px solid rgba(34,211,238,0.2)", borderRadius: 8, padding: "5px 12px", cursor: "pointer", fontWeight: 600 }}>
                    Full log →
                  </button>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  {auditLog.map((log, i) => (
                    <div
                      key={i}
                      style={{ display: "flex", gap: 12, padding: "12px 0", borderBottom: i < auditLog.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", alignItems: "flex-start" }}
                    >
                      <div style={{ width: 8, height: 8, borderRadius: "50%", background: tipoBullet[log.tipo], marginTop: 4, flexShrink: 0, boxShadow: `0 0 6px ${tipoBullet[log.tipo]}55` }} />
                      <div style={{ flex: 1 }}>
                        <div style={{ color: "#cbd5e1", fontSize: 12, fontWeight: 600 }}>{log.accion}</div>
                        {log.empresa !== "—" && (
                          <div style={{ color: "#475569", fontSize: 11, marginTop: 2 }}>{log.empresa}</div>
                        )}
                        <div style={{ display: "flex", gap: 8, marginTop: 3 }}>
                          <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: "#334155" }}>{log.ip}</span>
                          <span style={{ color: "#1e293b" }}>·</span>
                          <span style={{ fontSize: 10, color: "#334155" }}>{log.tiempo}</span>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              {quickActions.map((a) => (
                <div
                  key={a.label}
                  className="nav-item"
                  style={{ background: "rgba(14,165,233,0.04)", border: "1px solid rgba(34,211,238,0.1)", borderRadius: 12, padding: "16px 18px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}
                >
                  <span style={{ fontSize: 22 }}>{a.icon}</span>
                  <div>
                    <div style={{ color: "#e2e8f0", fontSize: 13, fontWeight: 600 }}>{a.label}</div>
                    <div style={{ color: "#475569", fontSize: 11 }}>{a.desc}</div>
                  </div>
                </div>
              ))}
            </div>
